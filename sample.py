...
from PIL import Image
from src.attention_controllers import AttentionStore
...
def view_images(images, save_path, base_count, num_rows=1, offset_ratio=0.02, step=None):
    ...
    base_count += 40
    if step is not None:
        pil_img.save(os.path.join(save_path, f"{base_count:05}_{step}.png"))
    else:
        pil_img.save(os.path.join(save_path, f"{base_count:05}.png"))
    # display(pil_img)
...
def aggregate_attention_all(prompts, attention_store: AttentionStore, res: int, from_where: List[str], is_cross: bool, select: int, len_tokens: int):
    attention_maps = attention_store.get_average_attention()

    out = []
    for _, value in attention_maps.items():
        ...
        for item in value:
            ...
            if res!=16:
                continue
            cross_maps_512 = torch.zeros((cross_maps.shape[0], 256, 256, 77))
            for token in range(len_tokens):
                token_cross_map = cross_maps[:, :, :, token].reshape(1, cross_maps.shape[0], res, res)
                token_cross_map = F.interpolate(token_cross_map, size=(256, 256), mode='bicubic').reshape(cross_maps.shape[0], 256, 256)
                cross_maps_512[:, :, :, token] = token_cross_map[:, :, :]
            out.append(cross_maps_512)
    out = torch.cat(out, dim=0)
    out = out.sum(0) / out.shape[0]
...
def aggregate_attention_all_step(prompts, attention_store: AttentionStore, res: int, from_where: List[str], is_cross: bool, select: int, len_tokens: int):
    attention_all_step_maps = attention_store.all_step_attention

    out_all_step = {}
    for step, attention_maps in attention_all_step_maps.items():
        out = []
        for _, value in attention_maps.items():
            for item in value:
                res = int(math.sqrt(item.shape[1]))
        ...
        if not absolute:
            out = (out - out.min()) / (out.max() - out.min() + 1e-8)
        out_all_step[step] = out.cpu()

    return out_all_step

def show_cross_attention(prompts, tokenizer, save_path, base_count, attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0):
    tokens = tokenizer.encode(prompts[select])
    decoder = tokenizer.decode
    
    attention_maps = aggregate_attention_all(prompts, attention_store, res, from_where, True, select, len(tokens))
    images = []
    for i in range(len(tokens)):
        image = attention_maps[:, :, i]
        image = 255 * image / image.max()
        image[image < 0] = 0 # process the overflow
        image = image.unsqueeze(-1).expand(*image.shape, 3)
        image = image.numpy().astype(np.uint8)
        image = np.array(Image.fromarray(image).resize((256, 256)))
        # tmp_image = Image.fromarray(image) 
        # tmp_image.save(os.path.join(save_path, '{}_{}.png'.format(base_count, decoder(int(tokens[i])))))
        text_under_image_string = decoder(int(tokens[i]))
        if text_under_image_string == '<new1>':
            text_under_image_string = 'V1*'
        elif text_under_image_string == '<new2>':
            text_under_image_string = 'V2*'
        image = text_under_image(image, text_under_image_string)
        # image = text_under_image(image, decoder(int(tokens[i])))

        images.append(image)
    view_images(np.stack(images, axis=0), save_path, base_count)

    if attention_store.all_step_attention_store:
        attention_maps_all_step = aggregate_attention_all_step(prompts, attention_store, res, from_where, True, select, len(tokens))
        for step, attention_maps in attention_maps_all_step.items():
            images = []
            for i in range(len(tokens)):
                image = attention_maps[:, :, i]
                image = 255 * image / image.max()
                ...
                image = text_under_image(image, decoder(int(tokens[i])))
                images.append(image)
            view_images(np.stack(images, axis=0), save_path, base_count, step=step)
...
def main():
    ...
    config.model.params.attention_store_background_mask = False
    config.model.params.attention_store_step = False
    config.model.params.check_token_attn = opt.check_token_attn
    config.model.params.all_step_attention_store = opt.all_step_attention_store
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        ...
                        if not opt.skip_save:
                            for x_sample in x_samples_ddim:
                                ...
                                base_count += 1
                            if opt.check_token_attn:
                                show_cross_attention(prompts, model.cond_stage_model.tokenizer, sample_path, base_count, model.controller, res=16, from_where=("up", "down"))
