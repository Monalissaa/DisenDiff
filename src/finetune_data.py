...
class MaskBase(Dataset):
    def __init__(self,
        ...
        # mask_datapath = '/home/mona/codes/Unified-codebase-for-proccessing-the-single-image/datasets/masks/cat'
        if mask_datapath is not None and len(mask_datapath)>0:
            self.mask_background = True
            if os.path.isdir(mask_datapath):
                self.mask_paths = [os.path.join(mask_datapath, file_path) for file_path in os.listdir(mask_datapath) if isimage(file_path)]
            self.mask_flip = transforms.RandomHorizontalFlip(p=1)
        else:
    ...
    def __getitem__(self, i):
        ...
        example["mask_background_image"] = np.ones((self.size // 8, self.size // 8))
        if i > self._length2 or self._length2 == 0:
            ...
            if self.mask_background:
                mask_background_image = Image.open(self.mask_paths[i % self._length1])
        ...
        if mask_background:
            mbi = np.array(mask_background_image).astype(np.uint8)
            crop = min(mbi.shape[0], mbi.shape[1])
            h, w, = mbi.shape[0], mbi.shape[1]

            mbi = mbi[(h - crop) // 2:(h + crop) // 2,
                    (w - crop) // 2:(w + crop) // 2]
            mask_background_image = Image.fromarray(mbi)            
            diff = ImageChops.difference(image_prev, image)
            if diff.getbbox():
                mask_background_image = self.mask_flip(mask_background_image)
            if self.size is not None:
                mask_background_image = mask_background_image.resize((self.size // 8, self.size // 8), resample=Image.NEAREST)
            
            mask_background_image = np.array(mask_background_image).astype(np.uint8)
            mask_background_image = (mask_background_image / 255).astype(np.float32)
            # print(mask_background_image[16,:])
            # exit(0)
            example["mask_background_image"] = mask_background_image
            # print(example["mask_background_image"].shape)
            # exit(0)
            # pil_image = Image.fromarray(mask_background_image)
            # pil_image.save('/home/mona/codes/Unified-codebase-for-proccessing-the-single-image/test/images/train_image/mask_image.jpg')
        ...
        # image = 255 * input_image1 / input_image1.max()
        ...
        example["image_input"] = input_image1
